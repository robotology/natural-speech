...
eval "echo \"$(sed "s:\":\\\\\":g" $HTK_COMMON/HTE.dnn.am)\"" >HTE.dnn.am
eval "echo \"$(sed "s:\":\\\\\":g" $HTK_COMMON/cvn.cfg)\"" >cvn.cfg
eval "echo \"$(sed "s:\":\\\\\":g" $HTK_COMMON/connect.hed)\"" >connect.hed
eval "echo \"$(sed "s:\":\\\\\":g" $HTK_COMMON/basic.htkc)\"" >basic.htkc
eval "echo \"$(sed "s:\":\\\\\":g" $HTK_COMMON/train.htkc)\"" >train.htkc
# ident_cvn
...
touch proto/foolist
HHEd -H $GMM_MODEL_DIR/$GMM_HMMDEFS_DIR/$GMM_HMMDEFS -M dnn6/init connect.hed $GMM_HMMLIST
if [ ! -d "cvn" ]; then
    mkdir -p cvn
fi
...
### Training  ####################
if [ ! -d "dnn6.trained" ]; then
    mkdir -p dnn6.trained
fi

date
HNTrainSGD -C basic.htkc -C train.htkc -H dnn6/init/$GMM_HMMDEFS -M dnn6.trained -S train.scp -N hv.scp -l LABEL -I alignment.mlf $GMM_HMMLIST
date
