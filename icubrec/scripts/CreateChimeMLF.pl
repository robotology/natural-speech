...
if ( @ARGV < 5 )
{
    print "$0 <training or test set> <MFC files> <transcription files> <dictionary> <output MLF> <output script> [ignore noises] [include OOVs] [unknown vocab file] [fix up] [find and replace file] [split file]\n";
    exit(1);
}
...
print "Training or test set:  $set\n";
...
if ($set eq 'train')
{
	# For chime training set, we have several channels
...
#		print "Line is: " . $line . "\n";
		...
		if (($line =~ /bad_recording/) || (scalar @words <= 1))
		{
				...
				for ($i = 0; $i < scalar @words; $i++)
				{
					if (length($words[$i]) > 0)
					{
						...
						if ($inDict{$words[$i]} == 1)
						{
							...
							{
								$words[$i] =~ s/([^A-Za-z0-9])/\\\1/g; #"\\" . $words[$i];
							}
